...
return_option_if_present_and_valid <- function(name, value, type, default, check, args, partial) {
  ...
  opt <- scratch_config_type_handler(type, name, partial)(opt)
  if (!is.null(check)) {
...
return_value_if_present_and_valid <-
  function(name, value, type, default, check, args, partial) {
    ...
    value <- scratch_config_type_handler(type, name, partial)(value)
    if (!is.null(check)) {
...
config_to_functions <- function(config, remove_prefix = TRUE) {
  ...
  for (k in seq_along(
    ...
    names(config[[k]]) <- paste0(".", names(config[[k]]))
    body(fn) <- substitute(
      {
        config_get(
          name = .name,
...
config_to_functions(scratch_config_load())
roptgen <- function(config, option_obj = paste(basename(getwd(), 'options', sep = '_')), inline = TRUE, out = paste(basename(getwd(), 'options', sep = '-')),
                    tests = TRUE) {
